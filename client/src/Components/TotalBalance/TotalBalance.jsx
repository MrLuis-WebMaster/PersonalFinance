...
const TotalBalance = ({ transactions, currency }) => {
  ...
  if (transactions) {
    operationEarning = transactions
      .filter(
        (e) => new Date(e.date).getMonth() + 1 === month && e.type === "Earning"
      )
      .reduce((a, b) => a + Number(b.amount), 0);
    operationExpense = transactions
      .filter(
        (e) => new Date(e.date).getMonth() + 1 === month && e.type === "Expense"
      )
      .reduce((a, b) => a + Number(b.amount), 0);
  }
          ...
          <div>
            <h1>
              ...
              {new Intl.NumberFormat().format(
                operationEarning - operationExpense
              )}{" "}
              {currency}
            </h1>
            <h2>Month : {monthArray[month - 1]}</h2>
          </div>
          <Box sx={{ display:"flex", width:{xs:'295px',sm:'400px'}, margin: "0 auto", paddingBottom:{xs:'3.5rem'}}}>
